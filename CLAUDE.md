...
- **`Header`** renders the panel title alongside a refresh button (top-right) that re-invokes `getActiveGames()` on click and spins its icon while the request is in flight.
