...
- **`get_active_games`** fetches the Insignia stats endpoint with a 60s in-process cache (`_cache`/`CACHE_TTL_SECONDS`) to avoid hammering the API, since the QAM panel may be opened frequently.
